...
class SettingsWireFormatTest {
        ...
        runTest {
            assertEquals(
                mapOf(
                    ...
                    StoredGridType.THREE_BY_THREE to
                        """{"common":{"grid_type":"THREE_BY_THREE"}}""",
        ...
        runTest {
            assertEquals(
                mapOf(
                    ...
                    StoredVideoQuality.UHD to
                        """{"modes":{"VIDEO":{"video_quality_back":"UHD"}}}""",
                    StoredVideoQuality.FHD to
                        """{"modes":{"VIDEO":{"video_quality_back":"FHD"}}}""",
    ...
    private suspend fun encode(settings: CameraSettings): String {
    ...
    private suspend fun decode(stored: String): CameraSettings {
        val prefs = settingsPrefsSerializer.readFrom(stored.encodeToByteArray().inputStream())

        return mapper.map(prefs.common)
    }

    private suspend fun encodeCommon(common: StoredCameraSettings): String {
        val output = ByteArrayOutputStream()
    ...
    private suspend fun encodeMode(mode: StoredModeSettings): String {
        val output = ByteArrayOutputStream()
    ...
    private suspend fun decodeMode(stored: String): StoredModeSettings {
        val prefs = settingsPrefsSerializer.readFrom(stored.encodeToByteArray().inputStream())

        return prefs.modes.getValue(MODE)
    }
